...
|---|---|---|---|
| 0:00 | Click **▶ Play the demo call**, then **2x** | "Same call, handled two ways. The Hard Call listens, and puts the reply-due date and the next question on the worker's screen while the customer is still on the phone." | Transcript starts. Right column: **"Coaching is off for this call."** — present from **+0.11 s**, before the first line |
